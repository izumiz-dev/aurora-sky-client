...
export function debugStorage() {
  ...
  // Check expiry
  const expiry = localStorage.getItem('bsky-session-expiry') || 
                sessionStorage.getItem('bsky-session-expiry');
  if (expiry) {
    ...
    console.log('Time left:', Math.floor((expiryDate.getTime() - now.getTime()) / 1000 / 60), 'minutes');
  }
  ...
  console.log('VITE_SESSION_KEY:', import.meta.env.VITE_SESSION_KEY ? 'configured' : 'not configured');
  
  // Test encryption
  console.log('\n--- Encryption Test ---');
  import('./crypto').then(({ SessionCrypto }) => {
    SessionCrypto.encrypt('test').then(() => {
