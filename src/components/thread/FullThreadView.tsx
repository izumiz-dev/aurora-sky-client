...
export const FullThreadView: FunctionComponent<FullThreadViewProps> = ({ post }) => {
  ...
  const flattenFullThread = (thread: ThreadViewPost): Post[] => {
    ...
    const collectAllReplies = (node: ThreadViewPost, depth = 0) => {
      if (node.replies && node.replies.length > 0 && depth < 10) {
        // 深さ制限
  ...
  if (!threadData?.data.thread) {
    return <div className="text-center py-8 text-white/60">スレッドが見つかりませんでした</div>;
  }
  ...
  return (
    <div className="relative space-y-0">
      ...
      {threadPosts.map((threadPost, index, array) => {
        ...
        const isDirectReply = prevPost && threadPost.record?.reply?.parent?.uri === prevPost.uri;

        return (
          <div key={threadPost.uri} className="relative">
            {/* 上部の接続線（アイコンの中心） */}
            {index > 0 && <div className="absolute left-[48px] -top-4 h-4 w-0.5 bg-white/30" />}

            {/* 投稿表示 */}
            <PostItem post={threadPost} hideReplyTo={isDirectReply || false} inModal={true} />
